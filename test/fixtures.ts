 ...
 * @param symbol NFT Contract Symbol Param
 */
export async function deployRirromHH(calypsoNFTAddress: string = ethers.ZeroAddress, name: string = "NFT Name", symbol: string = "NFT Symbol", destinationChainName: string = "staging-utter-unripe-menkar") {
    const [ signer ] = await ethers.getSigners();

    const factory = await ethers.getContractFactory("RirromNFT", signer);
